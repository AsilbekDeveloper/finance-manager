...
export default function Budgets() {
  ...
  const load = async () => {
    ...
    try {
      const [b, c] = await Promise.all([
        api.getBudgets(company.id),
        api.getCategories(company.id, "expense"),
      ]);
      setBudgets(b.data || []);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    await api.createBudget({ ...form, amount:parseFloat(form.amount), company_id:company.id });
    setSaving(false);
    setShowForm(false);
    ...
    <div className="fade-in">
      ...
      <div className="grid-3" style={{ marginBottom:20 }}>
        <div className="card" style={{ textAlign:"center" }}>
          <div style={{ color:"var(--text2)", fontSize:12, marginBottom:4 }}>Jami byudjet</div>
          <div style={{ color:"var(--blue)", fontWeight:700, fontSize:22 }}>{formatCurrency(totalBudget)}</div>
        </div>
        <div className="card" style={{ textAlign:"center" }}>
          <div style={{ color:"var(--text2)", fontSize:12, marginBottom:4 }}>Sarflangan</div>
          <div style={{ color:"var(--amber)", fontWeight:700, fontSize:22 }}>{formatCurrency(totalSpent)}</div>
        </div>
        <div className="card" style={{ textAlign:"center" }}>
          <div style={{ color:"var(--text2)", fontSize:12, marginBottom:4 }}>Limit oshgan</div>
          <div style={{ color: overCount>0?"var(--red)":"var(--green)", fontWeight:700, fontSize:22 }}>
            {overCount>0 ? `⚠️ ${overCount} ta` : "✅ Normal"}
          </div>
        </div>
      </div>
        ...
        <div className="card">
          <div className="empty-state" style={{ padding:60 }}>
            ...
            <p style={{ marginTop:8, fontSize:13, color:"var(--text2)" }}>
              Xarajat kategoriyalari uchun oylik limit belgilab,<br/>moliyangizni nazorat qiling
            </p>
          ...
          <div className="modal">
            ...
            <div className="flex gap-2" style={{ marginTop:8 }}>
              ...
              <button className="btn btn-primary" onClick={handleSave}
                disabled={saving||!form.category_id||!form.amount} style={{ flex:1, justifyContent:"center" }}>
