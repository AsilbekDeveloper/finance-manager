  ...
  const load = useCallback(async () => {
    ...
    try {
      const [overview, txns, analytics] = await Promise.all([
        api.getOverview(company.id, period),
        api.getTransactions(company.id, { limit: 7 }),
        api.getAnalytics(company.id),
      ]);
      setStats(overview);
      setRecent(txns.data || []);
      ...
      <div className="flex items-center justify-between" style={{ marginBottom:24 }}>
        <div>
          ...
          <p className="text-muted" style={{ marginTop:4 }}>
            {company?.name} — moliyaviy ko'rinish
          </p>
        </div>
        <div className="flex gap-2">
          {[["today","Bugun"],["week","Hafta"],["month","Oy"]].map(([val,lbl]) => (
            <button key={val} onClick={() => setPeriod(val)}
              className={`btn ${period===val ? "btn-primary" : "btn-ghost"}`}
              style={{ padding:"6px 14px" }}>{lbl}</button>
          ))}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 2fr", gap:16 }}>
        ...
        <div className="card">
          ...
          <div style={{ marginTop:16, padding:12, background:"var(--bg3)", borderRadius:8 }}>
            <p style={{ fontSize:12, color:"var(--text2)", textAlign:"center" }}>
              ...
              <strong style={{ color:"var(--blue)" }}>@uzfinx_bot</strong>
            </p>
          </div>
        </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between" style={{ marginBottom:16 }}>
            ...
            <a href="/transactions" style={{ fontSize:13, color:"var(--blue)", textDecoration:"none" }}>
              Barchasini ko'rish →
            </a>
          </div>
          {recent.length === 0 ? (
            <div className="empty-state" style={{ padding:30 }}>
              ...
              <p style={{ fontSize:12, marginTop:8, color:"var(--text3)" }}>
                Yuqoridagi tugmani bosib boshlang
              </p>
            </div>
          ) : (
            <table>
