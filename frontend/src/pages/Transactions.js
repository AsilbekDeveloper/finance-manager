...
import { Plus, Search, Filter, Pencil, Trash2, Send } from "lucide-react";
...
export default function Transactions() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.getTransactions({ ...params, limit: 200 });
      let data = r.data || [];
      if (filters.search) {
    ...
    <div className="fade-in">
      ...
      <div className="grid-3" style={{ marginBottom: 20 }}>
        <div className="card" style={{ textAlign: "center" }}>
          <div style={{ color: "var(--text2)", fontSize: 12 }}>Daromad</div>
          <div style={{ color: "var(--green)", fontWeight: 700, fontSize: 20 }}>+{formatCurrency(income)}</div>
        </div>
        <div className="card" style={{ textAlign: "center" }}>
          <div style={{ color: "var(--text2)", fontSize: 12 }}>Xarajat</div>
          <div style={{ color: "var(--red)", fontWeight: 700, fontSize: 20 }}>-{formatCurrency(expense)}</div>
        </div>
        <div className="card" style={{ textAlign: "center" }}>
          <div style={{ color: "var(--text2)", fontSize: 12 }}>Sof</div>
          <div style={{ color: income - expense >= 0 ? "var(--green)" : "var(--red)", fontWeight: 700, fontSize: 20 }}>
      ...
      <div className="card" style={{ marginBottom: 16, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
        ...
        <button className="btn btn-ghost" onClick={() => setFilters({ start_date: "", end_date: "", type: "", category_id: "", search: "" })}>
          Tozalash
        </button>
      </div>
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 13, color: "var(--text2)" }}>{total} ta natija</span>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <Send size={14} style={{ color: "var(--text3)" }} />
            <span style={{ fontSize: 12, color: "var(--text3)" }}>Telegram orqali ham qo'shish mumkin</span>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p style={{ marginTop: 8, fontSize: 13 }}>Filter shartlarini o'zgartiring yoki yangi tranzaksiya qo'shing</p>
          </div>
        ) : (
