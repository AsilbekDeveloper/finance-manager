...
const PieTooltip = ({ active, payload }) => {
  if (!active || !payload?.length) return null;
  return (
    <div style={{ background:"var(--bg3)", border:"1px solid var(--border)", borderRadius:8, padding:"8px 12px", fontSize:13 }}>
    ...
    <div className="fade-in">
      ...
      <div className="flex gap-2" style={{ marginBottom: 20 }}>
        {[
          ...
          { key: "expense", label: "🔴 Xarajat taqsimoti" },
          { key: "income", label: "💚 Daromad taqsimoti" },
          ...
          <div className="card">
            ...
            {monthly.every(m => m.income === 0 && m.expense === 0) ? (
              <div className="empty-state" style={{ padding:60 }}>
                ...
                <p>Bu yil uchun ma'lumot yo'q</p>
              </div>
            ) : (
      ...
      {tab === "expense" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
          <div className="card">
            <h3 style={{ marginBottom:16 }}>🥧 Xarajat taqsimoti (oy)</h3>
            {expCats.length === 0 ? (
              <div className="empty-state" style={{ padding:60 }}>
                <div className="empty-icon">📊</div>
                <p>Bu oy xarajat yo'q</p>
              </div>
            ) : (
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie data={expCats} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} label={({ name, percent }) => `${name} ${(percent*100).toFixed(0)}%`} labelLine={false} fontSize={11}>
      ...
      {tab === "income" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
          <div className="card">
            <h3 style={{ marginBottom:16 }}>🥧 Daromad taqsimoti (oy)</h3>
            {incCats.length === 0 ? (
              <div className="empty-state" style={{ padding:60 }}>
                <div className="empty-icon">📊</div>
                <p>Bu oy daromad yo'q</p>
              </div>
            ) : (
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie data={incCats} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} label={({ name, percent }) => `${name} ${(percent*100).toFixed(0)}%`} labelLine={false} fontSize={11}>
          ...
          {netData.every(m => m.net === 0) ? (
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={netData}>
                ...
                <YAxis stroke="var(--text3)" fontSize={11} tickFormatter={v => (v/1000000).toFixed(1)+"M"} />
                <Tooltip content={<CustomTooltip />} />
                <Line type="monotone" dataKey="net" stroke="#6366f1" strokeWidth={2} dot={{ fill:"#6366f1", r:4 }} name="Sof foyda" />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
