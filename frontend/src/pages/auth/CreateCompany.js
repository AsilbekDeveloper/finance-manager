...
export default function CreateCompany() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      console.log("SESSION DEBUG (CreateCompany):", res);
    };
    checkSession();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!session?.access_token) {
        setErr("Auth sessiyasi topilmadi. Iltimos, qayta kiriting.");
        return;
      }

      const res = await api.createCompany(name.trim());

      console.log("✅ COMPANY CREATED:", res);

      await loadCompanies();

      // Active company saqlash
      const companyId = res.id || res.company_id;
      if (companyId) {
      ...
      setErr(e.message || "Kompaniya yaratishda xato yuz berdi. Qayta urinib ko‘ring.");
    } finally {
      setLoading(false);
    }
  };
